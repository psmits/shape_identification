...
trm <- is.na(adult_scales$scute)

adult_scales <- adult_scales %>% drop_na(scute)
...
scute_split <- split.land(scute_shape, factor(adult_scales$scute))
...
null <- rerun(1000, rand_meandist(scute_shape, factor(adult_scales$scute))) %>%
...
for(ii in seq(length(gg))) {
  ...
  scute_split <- 
    split.land(scute_shape, factor(short$scute))

  scute_mean <- map(scute_split, mshape)
  scute_mean_dist <- procdist(scute_mean[[1]], scute_mean[[2]])

  null <- rerun(1000, rand_meandist(scute_shape, 
                                    factor(adult_scales$scute))) %>%
    tibble(null = .) %>%                 # rerun is type safe
    unnest()                             # need to unpack list column
...
walk2(out$graph, gg,
     ~ ggsave(filename = paste0(here::here('doc', 'figure'), 
                                '/scales_test_hist_', .y, '.pdf'),
              plot = .x,
              width = 10, height = 8))
