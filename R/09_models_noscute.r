...
source(here::here('R', 'helper10_train_test_noscute.r'))
...
source(here::here('R', '08_supervised_noscute.r'))
...
for(ii in seq(length(meth))) {
  results[[ii]] <- use.model_noscute(method = meth[ii],
                                     adult = adult, 
                                     scheme = schemes,
                                     npred = npred)
}
...
ggsave(plot = roc.plot, filename = here::here('doc', 'figure', 'emys_model_sel_noscute.pdf'),
       width = 7.5, height = 8)
...
ggsave(plot = oos.plot, filename = here::here('doc', 'figure', 'emys_oos_sel_noscute.pdf'),
       width = 7.5, height = 4)
