...
write_csv(nsp_table,
          path = here::here('doc', 'class_counts_noscute.csv'))




cliped <- adult[, c('spec', 'lat', 'long', 
                    'sp10.1', 'sp10.2', 'sp10.3', 
                    'sp14.1', 'sp14.2', 
                    'morph')]
write.csv(cliped, file = here::here('data', 'clean_meta_marm_noscute.csv'), 
          fileEncoding = 'UTF-16LE')

# write landmarks
writeland.tps(A = land.adult,
              file = here::here('data', 'mamorota_clean_land_noscute.tps'))

# full data print outs
write_rds(adult, path = here::here('data', 'turtle_plastron_data_clean_noscute.rds'))
write_rds(fit,
          path = here::here('data', 'turtle_plastron_gpa_noscute.rds'))
