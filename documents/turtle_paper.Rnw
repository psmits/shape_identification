...
The optimal multinomial logistic regression models for all four classification schemes, based on AICc, have all \Sexpr{length(tm.a.analysis$best$spinks$coefnames)} possible PCs as features(Table SUPPLEMENT). The second best models for all classification schemes have \Sexpr{length(tm.a.analysis$best$spinks$coefnames) - 1} PCs as features. The \(\delta\)AICc values between the optimal and second best model range from \Sexpr{min(daic)} for the first morphological based classification hypothesis to \Sexpr{max(daic)} for the second molecular based classification hypothesis. 
...
The selected number of features in the optimal random forest models for each classification scheme vary much more than in the case of the multinomial logistic regression models (Fig. \ref{fig:roc}), ranging from \Sexpr{min(oprf)} for the first morphological based classification hypothesis to \Sexpr{max(oprf)} for the second morphological based classification hypothesis. 
...
The results of training the random forest model also include the variable importance for best separating the different classes. Recursive feature selection of the best random forest model of the chosen classification scheme indicated that after \Sexpr{trf.a$spinks$optsize} PCs were included as features, AUC ROC would not increase. Of these \Sexpr{trf.a$spinks$optsize} features, three are illustrated here (Fig. \ref{fig:imp_pc}) the first two of which are most important SUPPLEMENT WITH VARIABLE IMPORTANCE INFORMATION?. 
