...
%   gap statistic
The optimal number of clusters of shape configurations is unknown being possibly three, four, or some other unknown. Clustering solutions were estimated for between 1 and \Sexpr{nrow(tadult.gap$Tab)} clusters. Clustering solutions were compared using the gap statistic, which is a measure of goodness of clustering CITATION.
