...
\begin{frame}
  ...
  % two columns, text on one side and idealized landmarks on the other
  % ken probably has an image for the landmarks
  % these landmarks have been used before in other studies
  \begin{columns}
