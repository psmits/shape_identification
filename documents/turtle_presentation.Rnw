...
tpsgrid(turtle.land.adult[, , fst.max], mt)
tpsgrid(turtle.land.adult[, , fst.min], mt)

tpsgrid(turtle.land.adult[, , snd.max], mt)
tpsgrid(turtle.land.adult[, , snd.min], mt)

imp <- cbind(turtle.adult[, most.imp[1:2]], sp = turtle.adult$spinks)
imp.pca <- ggplot(imp, aes(x = PC3, y = PC8, colour = sp)) + geom_point()
imp.pca <- imp.pca + scale_colour_manual(name = '', values = cbp)
imp.pca
@

% show tps grids to the side of the PCA plot of the first two most important axes
\begin{figure}[h]
  \centering
  \hspace*{\fill}
