...
In addition to the \textit{E. marmorata/pallida} case, we also analyzed a set of seven morphologically distinct species in order to test if the approaches used here actually work. The seven species analyzed were \textit{Emys blandigii}, \textit{Terrapene coahuila}, \textit{Clemmys guttata}, \textit{Glyptemys insculpta}, \textit{Glyptemys muhlenbergii}, \textit{Emys orbicularis}, and \textit{Terrapene ornata} with a total of XXX specimens analyzed in total. Just as in the \textit{E. marmorata/pallida} example, we analyzed plastron shape variation. The support for a classification scheme with seven categories was evaluated using random forest, linear discriminate analysis, and multinomial logistic regression where the number of PCs used as predictors ranged from 2 to 11. Data was split into training and testing sets and model performance for both sets was evaluated using the AUC metric, just as above.
...
\section{Comparison with clear-cut example}
In contrast with the above results, the analysis of the clearly distinct species SUPPLEMENT demonstrates almost perfect classification of the both the in-sample and out-of-sample datasets (Table \ref{tab:second_res}). These results demonstrate that if there were strong distinctions between the states of the classification schemes, the methods used here would be able to recover this.
